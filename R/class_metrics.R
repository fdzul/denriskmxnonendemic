    ...
    table |>
        dplyr::bind_rows(tibble::tibble(.metric = c("auc"),
                         ...
                         tibble::tibble(.metric = c("BCI"),
                                        ...
                                        difference = round(train-test, 2)),
                         tibble::tibble(.metric = c("Brier Score"),
                                        .estimator = c("binary"),
                                        train = round(yardstick::brier_class_vec(y_class$Real, y_class$.pred_presence),2),
                                        test = round(yardstick::brier_class_vec(x_class$Real, x_class$.pred_presence),2),
                                        difference = round(train-test, 2)),
                         tibble::tibble(.metric = c("AUC PR"),
                                        .estimator = c("binary"),
                                        train = round(yardstick::pr_auc_vec(y_class$Real, y_class$.pred_presence),2),
                                        test = round(yardstick::pr_auc_vec(x_class$Real, x_class$.pred_presence),2),
                                        difference = round(train-test, 2)),
